    ...
    class dObjetoEncontrado
    {
        ...
        public string Insertar(ObjetoEncontrado o)
        {
            try
            {
                ...
                string insert = string.Format("Insert into Obj_Perdido(Nombre_Objeto) values('{0}')", o.NombreObjetoEncontrado);
        ...
        public string Modificar(ObjetoEncontrado o)
        {
            try
            {
                //TODO: :)
                SqlConnection con = db.ConectaDb();
                string update = string.Format("UPDATE Obj_Perdido SET Nombre_Objeto='{0}' where IdObjPerdido={1}", o.NombreObjetoEncontrado, o.IdObjetoEncontrado);
        ...
        public ObjetoEncontrado BuscarPorId(int id)
        {
            try
            {
                ...
                SqlConnection con = db.ConectaDb();
                string select = string.Format("select IdObjPerdido,Nombre_Objeto from Obj_Perdido where IdObjPerdido={0}", id);
        ...
        public List<ObjetoEncontrado> ListarTodo()
        {
            try
            {
                ...
                ObjetoEncontrado objeto = null;
                SqlConnection con = db.ConectaDb();
                SqlCommand cmd = new SqlCommand("select IdObjPerdido,Nombre_Objeto from Obj_Perdido", con);
                SqlDataReader reader = cmd.ExecuteReader();
                while (reader.Read())
                {
                    objeto = new ObjetoEncontrado();
